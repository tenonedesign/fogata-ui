<script>
  ...
  import { SettingsSharp } from 'svelte-ionicons';
    ...
    <Card>
      <h1 class=" text-5xl sm:text-6xl lg:text-[80px] mt-8 text-center font-semibold tooltip tooltip-secondary flex-none" data-tip="{($pool.apy).toLocaleString($user.language, {minimumFractionDigits:8})}%">{$pool.apy.toFixed(2)}%</h1>
